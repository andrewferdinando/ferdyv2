...
export default function ContentLibraryPage() {
  ...
  const handleEditAsset = async (asset: Asset) => {
    try {
      ...
      // Clear tags to move asset to "Needs Attention"
      await saveAssetTags(asset.id, [])
            ...
            {activeTab === 'ready' && (
              <div className="mb-6">
                <div className="relative w-full max-w-md">
                  <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 h-4 w-4 pointer-events-none" />
                  <input
                    type="text"
                    placeholder="Search content..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full h-10 px-3 py-2.5 pl-12 border border-gray-300 rounded-lg text-sm leading-normal focus:border-[#6366F1] focus:ring-4 focus:ring-[#EEF2FF] focus:outline-none transition-all duration-150"
                  />
                </div>
              </div>
            )}
          ...
          <div className="p-4 sm:p-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                {/* Tag Selector */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-950 mb-3">Tags <span className="text-red-500">*</span></h3>
                  <TagSelector
                    brandId={brandId}
