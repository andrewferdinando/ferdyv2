...
function NonCustomersTab() {
  ...
  async function handleSyncToResend() {
    setSyncing(true)
    setSyncResult(null)
    const data = await apiFetch('/api/newsletter/contacts', { method: 'PATCH' })
    setSyncing(false)
    if (data.success) {
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          {contacts.filter(c => !c.resend_contact_id).length > 0 && (
            <p className="text-xs text-amber-600 mt-0.5">{contacts.filter(c => !c.resend_contact_id).length} not yet synced to Resend</p>
          )}
        </div>
        <div className="flex items-center gap-2">
          {contacts.some(c => !c.resend_contact_id) && (
            <button
              ...
              disabled={syncing}
              className="inline-flex items-center gap-2 px-4 py-2 border border-[#6366F1] text-[#6366F1] text-sm font-medium rounded-lg hover:bg-indigo-50 transition-colors disabled:opacity-50"
            >
              {syncing ? (
                <>
      ...
      {syncResult && (
        <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-700">
          Synced {syncResult.synced} contact{syncResult.synced !== 1 ? 's' : ''} to Resend.
          {syncResult.errors > 0 && (
            <span className="text-red-600"> {syncResult.errors} error{syncResult.errors !== 1 ? 's' : ''}.</span>
          )}
        </div>
      )}
