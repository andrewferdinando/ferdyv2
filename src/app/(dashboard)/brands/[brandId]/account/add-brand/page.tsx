...
export default function AddBrandPage() {
  ...
  const validate = useCallback((values: BrandFormValues) => {
    ...
    if (!result.success) {
      const issues = result.error.issues.reduce((acc, issue) => {
        ...
        if (path) acc[path] = issue.message
        return acc
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setTouched({ name: true, websiteUrl: true, countryCode: true, timezone: true })
  ...
  const handleSkipTeamAssignment = () => {
    showToast({ title: 'Brand created successfully.', type: 'success' })
    router.push('/brands')
  }
  ...
  const handleSaveTeamAssignment = async () => {
    ...
    try {
      ...
      showToast({ title: 'Failed to assign team members', type: 'error' })
    } finally {
      setIsSubmitting(false)
    }
  }
      ...
      return (
        <div className="max-w-xl mx-auto bg-white border border-gray-200 rounded-2xl p-8 text-center space-y-4">
          ...
          <p className="text-sm text-gray-600">
            Only Group Owners and Group Admins can add new brands. Ask your Group Owner to update your role if you need this access.
          </p>
          <button
