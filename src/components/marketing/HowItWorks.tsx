const steps = [
  {
    number: '01',
    title: 'Strategy & setup session',
    description:
      'A free one-on-one with a Ferdy strategist to plan your posting structure and load in your assets.',
  ...
  },
  {
    ...
    description:
      'Posts are generated using your structure and approved assets, then scheduled on a daily, weekly or monthly rhythm.',
  ...
  },
  {
    ...
    description:
      'Review, edit or skip any post before it publishes. Nothing goes out without your sign-off.',
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-7xl mx-auto relative">
          {steps.map((step, i) => (
            <div key={i} className="relative">
              <div
                className={`${step.bgColor} rounded-2xl p-10 h-full border border-gray-200 hover:shadow-xl transition-all hover:-translate-y-1`}
              >
                <div className="flex justify-between items-start mb-6">
                  <div className={`text-6xl font-bold ${step.numberColor}`}>{step.number}</div>
                  {step.badge && (
                    <span className="inline-flex items-center px-3 py-1 rounded-full bg-emerald-500 text-white text-xs font-bold tracking-wide shadow-sm">
                      {step.badge}
                    </span>
                  )}
                  {step.hasLock && (
