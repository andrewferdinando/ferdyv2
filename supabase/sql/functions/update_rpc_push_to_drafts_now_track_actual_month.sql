...
BEGIN
    ...
    BEGIN
        ...
        LOOP
            ...
            -- Track the actual month of drafts created (use first month encountered)
            IF v_pushed_month IS NULL THEN
                v_pushed_month := date_trunc('month', v_target.scheduled_at)::date;
            END IF;
        ...
        -- Update run with actual month pushed (if drafts were created)
        IF v_pushed_month IS NOT NULL THEN
            UPDATE runs
            SET target_month = v_pushed_month
            WHERE id = v_run_id;
        END IF;
