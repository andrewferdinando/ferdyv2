...
- **Lazy asset URL loading:** `useAssets` uses a two-phase approach — Phase 1 returns metadata instantly (page renders with placeholders), Phase 2 resolves all signed URLs in parallel in the background and updates state. URLs use a shared 9-minute in-memory cache. See `image_processing.md` for details.
