...
- **Public asset URLs:** The `ferdy-assets` bucket is public. `useAssets` calls `resolveAssetUrls()` synchronously in `mapAsset()` — every asset has URLs on first render with zero API calls. Grid thumbnails use Vercel's `/_next/image` optimizer (384px WebP, CDN-cached). See `image_processing.md` for details.
